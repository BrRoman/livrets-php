...
// Calcul de la date de Pâques en fonction de l'année :
...
function calculate_lit_day(date){
    ...
    if(date > pentecote && date < christ_roi){
        ...
        lit_day["lit_day_ref"] = "pa_" + dim_per_annum + "_" + weekday;
        if(weekday != 0){
            lit_day["lit_day_letters"] = days_fr[weekday] + " de la\\par " + latine_numbers[dim_per_annum] + "\\textsuperscript{e} semaine\\par du Temps Ordinaire";
        }
        else{
            lit_day["lit_day_letters"] = latine_numbers[dim_per_annum] + "\\textsuperscript{e} Dimanche\\par du Temps Ordinaire";
        }
    }
